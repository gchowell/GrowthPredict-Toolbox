...
| `parameters-rollingwindow-… .csv`     | Fit / Forecast | Parameter estimates and 95% CIs per window for the chosen growth model.          | `time`, then for each parameter *p*: `p mean`, `p 95% CI LB`, `p 95% CI UB`.                 |
| `MCSEs-rollingwindow-… .csv`          | Fit / Forecast | Monte Carlo standard errors per parameter per window (from bootstrap).           | `time`, `p MCSE` columns.                                                                    |
| `SCIs-rollingwindow-… .csv`           | Fit / Forecast | Identifiability span (SCI) per parameter.                                        | `time`, then `p SCI` where `SCI = log10(UB/LB)` (smaller ⇒ tighter ID).                      |
...
| `performance-calibration-… .csv`      | Fit / Forecast | In-sample (calibration-window) metrics.                                          | `window/time`, `MAE`, `RMSE`, `MAPE`, `PI_coverage`, `PI_width`, `WIS` (exact set may vary). |
...
| `Rt-… .csv` *(if computed)*           | Fit / Forecast | Effective reproduction number series derived from the fitted model & GI options. | `time`, `Rt_mean`/`Rt_median`, quantiles (e.g., `q0.025`, `q0.975`).                         |
...
| `parameters-GrowthModel-… .mat`       | Fit / Forecast | MATLAB struct snapshot for downstream plotting/forecasting.                      | Model choice, parameter draws, bootstrap artifacts, run settings.                            |
| `Forecast-… .mat`                     | **Forecast**   | MATLAB struct of forecast draws & summaries.                                     | Forecast paths, quantiles, horizon metadata.                                                 |
| `Fit-… .png` / `.pdf`                 | Fit            | Saved figure: best fit vs data (with CIs where applicable).                      | Visualization generated by your plotting helpers.                                            |
| `Forecast-… .png` / `.pdf`            | **Forecast**   | Saved figure: forecast with prediction intervals.                                | Visualization generated by your plotting helpers.                                            |
| `Performance-… .png` / `.pdf`         | **Forecast**   | Saved figure: accuracy metrics by horizon/window.                                | WIS / MAE / RMSE plots (as available).                                                       |
